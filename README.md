...
The Worker routes are exactly `spacegho.st/mods/ffxiv/term/vote*`,
`spacegho.st/mods/ffxiv/term/gallery*` and `spacegho.st/mods/ffxiv/almanac*`. No other path on the zone is touched. The only bindings
...
| `src/almanac.js` | The Almanac leaderboard: submission, cached aggregates, the owner's moderation |
| `src/almanac-board.js` | Pure aggregation: tiers, GPU families, per-submitter medians, outlier trimming, ranking, `recommendations.json` |
| `src/jsonschema.js` | A strict JSON Schema subset validator (unknown fields and unsupported keywords are errors) |
| `src/almanac-schema.js` | Generated by `scripts/build.js` from `public/mods/ffxiv/almanac/schema/results.v1.json` |
| `public/mods/ffxiv/almanac/` | The leaderboard page (`index.html`, `almanac.js`, `almanac.css`) and `schema/` (results and recommendations JSON Schemas) |
...
Players of **Almanac** (the open-source local AI companion for FFXIV,
[github.com/Spaceghost/almanac-dalamud](https://github.com/Spaceghost/almanac-dalamud)) run its
benchmark suite on their own model and hardware and may share the result. The page at
`https://spacegho.st/mods/ffxiv/almanac/` ranks models per VRAM tier and GPU family and plots
score against speed, so players can pick the best model for the memory and speed they have.
Everything is in D1; there is no KV or R2 for the leaderboard.
...
| GET | `/mods/ffxiv/term/vote/api/admin/almanac` | Owner only: newest 300 results (submitter digest shown as 8 hex characters), the suites with their pinned hash, totals |
| POST | `/mods/ffxiv/term/vote/api/admin/almanac/review` | Owner only, `{id, action}`: `hide`, `unhide`, `delete` |
| POST | `/mods/ffxiv/term/vote/api/admin/almanac/suite` | Owner only, `{suite_id, suite_version, deprecated: true\|false}`. A deprecated suite leaves the leaderboard and the recommendations, and new results for it are refused |

**Aggregation (computed on read).** A miss reads the newest 20,000 visible results of
non-deprecated suites (one D1 batch) and builds both JSON files; each is kept for 5 minutes in
the Cache API of that Cloudflare location and in browsers, so the leaderboard costs at most one
D1 read per location every 5 minutes, and there is no cron. Moderation drops the cached copies
in the owner's location; other locations catch up within 5 minutes. Within a bucket each
submitter counts once (the median of their runs); with 4 or more submitters, one more than 3.5
robust standard deviations (1.4826 x MAD) from the median score or log tokens/s is trimmed;
confidence is low below 3 submitters, medium to 9, high from 10. Ranking puts models under 10
tokens/s last and otherwise orders by the median score shrunk toward the tier's median by 2
pseudo-submitters, so one run cannot top a tier. Tiers: under 2 GB, 4-6, 8, 10-12, 16, 20-24,
32+ GB (boundaries at 2048, 7168, 9216, 13312, 17408 and 26624 MB, so a card reporting a little
under its nominal size stays in its tier).

**Limits and privacy.** 12 results an hour and 40 a day per address, 500 a day for the whole
site; one D1 read decides (and catches duplicates) before anything is written. The IP address
is never stored: `sha256('almanac-results:' + (GALLERY_SECRET or SESSION_SECRET) + ':' + ip)`
is kept with each result, for the limits and so each submitter counts once per model; the
owner sees its first 8 characters. `almanac_submit_log` rows older than a day are pruned by the
next submission. The suite hash is pinned by the first accepted submission for a version; if a
wrong hash is pinned, deprecate that version (or delete its `almanac_suites` row with
`wrangler d1 execute`) and publish the suite under a new version.

Tested: `node --test` (`tests/almanac.test.js`) covers the validator, schema conformance of
`recommendations.json`, the routes, submission, duplicates, suite pinning, the size, type,
origin, per-address and site-wide limits, tiers and GPU families, per-submitter collapsing,
outlier trimming, ranking, the cached reads and all moderation actions with cache invalidation.
...
```sh
npx wrangler d1 execute ghostty-vote --remote --file migrations/0006_almanac.sql
npx wrangler deploy                    # adds the spacegho.st/mods/ffxiv/almanac* route
curl -s https://spacegho.st/mods/ffxiv/almanac/recommendations.json | head -c 300
```
